...
class ModelPlain(ModelBase):
    ...
    def netG_forward(self):
        self.ir, self.vi = self.netG(self.L, self.H)
        if self.is_mask:
    ...
    def optimize_parameters(self, current_step):
        ...
        self.netG_forward()
        G_loss = self.G_lossfn_weight * self.G_lossfn(self.ir, self.L) + \
                 self.G_lossfn_weight * self.G_lossfn(self.vi, self.H)
        G_loss.backward()
    ...
    def current_visuals(self, need_H=True):
        ...
        out_dict['ir'] = self.ir.detach()[0].float().cpu()
    ...
    def current_results(self, need_H=True):
        ...
        out_dict['ir'] = self.E.detach().float().cpu()
        out_dict['vi'] = self.E.detach().float().cpu()
        if need_H:
