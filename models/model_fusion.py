...
class ModelFusion(ModelBase):
    """Train with pixel loss"""

    def __init__(self, opt):
        super(ModelFusion, self).__init__(opt)
        # ------------------------------------
        # define network
        # ------------------------------------
    ...
    def feed_data(self, data, need_H=True):
        ...
        if 'mask' in data:
            ...
            print('maskmaskmaskmaskmaskmaskmaskmask')
            self.is_mask = True
        else:
            self.is_mask = False
    ...
    def netG_forward(self):
        self.fusion = self.netG(self.L, self.H)
        if self.is_mask:
            print('is_maskis_maskis_maskis_maskis_mask')
            self.fusion = torch.mul(self.fusion, self.mask)
    ...
    def optimize_parameters(self, current_step):
        ...
        self.netG_forward()
        # print('q111111111111111', self.L.shape)
        # print('q222222222222222', self.H.shape)
        # print('q222222222222222', self.fusion.shape)
        G_loss, loss_gradient, loss_l1, loss_SSIM = self.G_lossfn(self.L, self.H, self.fusion)
        G_loss.backward()
        ...
        self.log_dict['loss_l1'] = loss_l1.item()
        self.log_dict['loss_SSIM'] = loss_SSIM.item()
        self.loss_value.append(G_loss.item())
        # self.loss_value.append(loss_gradient.item())
        # self.loss_value.append(loss_l1.item())
        # self.loss_value.append(loss_SSIM.item())
    ...
    def current_visuals(self, need_H=True):
        ...
        out_dict['fusion'] = self.fusion.detach()[0].float().cpu()
        # out_dict['vi'] = self.vi.detach()[0].float().cpu()
        if need_H:
    ...
    def current_results(self, need_H=True):
        ...
        out_dict['fusion'] = self.fusion.detach().float().cpu()
        if need_H:
