    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 talking_heads=talking_heads, use_attn_fn=use_attn_fn, head_scale=head_scale,
                                 on_attn=on_attn,
                                 **kwargs)
...
class CMIM(nn.Module):
    ...
    def forward(self, input_feats):
        ...
        return feats_V.reshape(bs, self.channel, H * W).permute(0, 2, 1)
...
class Encoder(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
                 ...
                 talking_heads=False, use_attn_fn=False, head_scale=False, on_attn=False, opt=None, nd_deconder=True,
                 **kwargs):
        super().__init__()
        ...
        #####################################################################################################
        ######################################### Feature Embedding #########################################
        self.conv_first_ir = nn.Conv2d(num_in_ch, embed_dim, 1)
        self.conv_first_vi = nn.Conv2d(num_in_ch, embed_dim, 1)
        ...
        self.patch_embed_vi = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=embed_dim, embed_dim=embed_dim,
            norm_layer=norm_layer if self.patch_norm else None)
        num_patches = self.patch_embed_ir.num_patches
        patches_resolution = self.patch_embed_ir.patches_resolution
        self.patches_resolution = patches_resolution
        ...
        self.CMIMs = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer = MSTB(dim=embed_dim,
            ...
            self.CMIMs.append(CMIM(embed_dim * 2, 2))
            self.encoder_ir.append(layer)
    ...
    def forward(self, ir, vi, size):
        ir = self.check_image_size(ir)
        vi = self.check_image_size(vi)
        ir = self.conv_first_ir(ir)
        vi = self.conv_first_vi(vi)
        B, C, H, W = ir.shape

        ir = self.patch_embed_ir(ir)
        vi = self.patch_embed_vi(vi)
        if self.ape:
            ir = ir + self.absolute_pos_embed
            vi = vi + self.absolute_pos_embed
        ir = self.pos_drop(ir)
        vi = self.pos_drop(vi)

        for i_layer in range(self.num_layers):
            ir = self.encoder_ir[i_layer](ir, size)
            vi = self.encoder_vi[i_layer](vi, size)
            x_CMIM = self.CMIMs[i_layer](torch.cat([ir, vi], dim=-1).view(B, H, W, 2 * C))
            ir = ir + x_CMIM
            vi = vi + x_CMIM
...
class Decoder(nn.Module):
    ...
    def __init__(self, embed_dim,
        ...
        self.norm_ir = norm_layer(embed_dim)
        self.norm_vi = norm_layer(embed_dim)
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]

        self.decoder_ir = nn.ModuleList()
        for i_layer in range(len(depths)):
            layer = STB(dim=embed_dim,
                        input_resolution=(patches_resolution[0],
                                          patches_resolution[1]),
                        depth=depths[i_layer],
                        num_heads=num_heads[i_layer],
            ...
            self.decoder_ir.append(layer)

        self.decoder_vi = nn.ModuleList()
        for i_layer in range(len(depths)):
            layer = STB(dim=embed_dim,
                        input_resolution=(patches_resolution[0],
                                          patches_resolution[1]),
            ...
            self.decoder_vi.append(layer)
        # merge non-overlapping patches into image
        self.patch_unembed_ir = PatchUnEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=embed_dim, embed_dim=embed_dim,
            norm_layer=norm_layer)
        self.patch_unembed_vi = PatchUnEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=embed_dim, embed_dim=embed_dim,
            norm_layer=norm_layer)

        self.conv_last_ir = nn.Conv2d(embed_dim, 1, 3, 1, 1)
        self.conv_last_vi = nn.Conv2d(embed_dim, 1, 3, 1, 1)

    def forward(self, ir, vi, size):
        H, W = size
        for i_layer in range(self.num_layers // 2):
            ir = self.decoder_ir[i_layer](ir, size)
            vi = self.decoder_vi[i_layer](vi, size)

        ir = self.norm_ir(ir)
        vi = self.norm_vi(vi)
        ir = self.patch_unembed_ir(ir, size)
        vi = self.patch_unembed_vi(vi, size)
        ir = self.conv_last_ir(ir)
        vi = self.conv_last_vi(vi)
        return ir[:, :, :H, :W], vi[:, :, :H, :W]


class MaskedSwinFusion(nn.Module):
    r""" SwinIR
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=1,
                 ...
                 talking_heads=False, use_attn_fn=False, head_scale=False, on_attn=False, opt=None, nd_deconder=True,
                 **kwargs):
        super(MaskedSwinFusion, self).__init__()
        self.embed_dim = embed_dim

        self.encoder = Encoder(img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
                               ...
                               use_checkpoint=use_checkpoint, upscale=upscale, img_range=img_range, upsampler=upsampler,
                               talking_heads=talking_heads, use_attn_fn=use_attn_fn, head_scale=head_scale,
                               on_attn=on_attn, opt=opt, **kwargs)
        self.nd_deconder = nd_deconder
        if self.nd_deconder:
            self.decoder = Decoder(embed_dim=embed_dim, patches_resolution=self.encoder.patches_resolution,
                                   depths=[2,2],
                                   num_heads=[6,6], window_size=window_size, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                                   qk_scale=qk_scale, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate,
                                   drop_path_rate=drop_path_rate, norm_layer=norm_layer, use_checkpoint=use_checkpoint,
                                   img_size=img_size, patch_size=patch_size, num_layers=2)


        else:
            self.proj = nn.Conv2d(embed_dim * 2, embed_dim, 3, 1, 1)
            self.image_fusion = nn.ModuleList()
            depths = [2,2]
            dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]
            for i_layer in range(len(depths)):
                layer = DDT(dim=embed_dim,
                            input_resolution=(self.encoder.patches_resolution[0],
                                              self.encoder.patches_resolution[1]),
                            depth=depths[i_layer],
                            num_heads=num_heads[i_layer],
                            window_size=window_size,
                            mlp_ratio=mlp_ratio,
                ...
                self.image_fusion.append(layer)
            ...
            self.patch_unembed_fusion = PatchUnEmbed(
                img_size=img_size, patch_size=patch_size, in_chans=embed_dim, embed_dim=embed_dim,
                norm_layer=norm_layer)
            self.conv_last_fusion = nn.Conv2d(embed_dim, 1, 3, 1, 1)
    ...
    def forward(self, ir, vi):
        size = (ir.shape[2], ir.shape[3])
        b, c, h, w = ir.shape
        ir, vi = self.encoder(ir, vi, size)
        if self.nd_deconder:
            ir, vi = self.decoder(ir, vi, size)
            return ir, vi
        else:
            # print(torch.cat([ir, vi], dim=-1).shape)
            imge_fusion = self.proj(
                torch.cat([ir, vi], dim=-1).permute(0, 2, 1).reshape(b, self.embed_dim*2, h, w )).reshape(
                b, self.embed_dim, h*w ).permute(0, 2, 1)
            for i_layer in range(len(self.image_fusion)):
                imge_fusion = self.image_fusion[i_layer](imge_fusion, size)
            imge_fusion = self.norm_fusion(imge_fusion)
            imge_fusion = self.patch_unembed_fusion(imge_fusion, size)
            imge_fusion = self.conv_last_fusion(imge_fusion)
            return imge_fusion[:, :, :size[0], :size[1]]
